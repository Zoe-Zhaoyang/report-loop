...
def _delivery_suffix(contract: ReportOutputContract) -> str:
    return (
        ...
        "为降低长响应被上游中断的风险，先立即用文件工具创建报告的三段式骨架，"
        "再分段补全正文并自检；不要先在回复中完整起草后再一次性写入。"
        "确认文件完整落盘后再结束。"
    )
...
def _build_batch_config(
    ...
    generation_dir: Path,
    command: tuple[str, ...],
    compact_single_case: bool = False,
) -> BatchConfig:
    return BatchConfig(
        command=command,
        ...
        flat_single_case=compact_single_case,
    )
...
def _persist_result(
    ...
    write_json(
        ...
        result.to_dict(include_report_text=include_report_text),
    )
...
def _case_attempt_run_id(wb_case_id: str, attempt: int, compact: bool = False) -> str:
...
def _execute_case_attempt(
    ...
    wb_run_id = _case_attempt_run_id(
        case.case_id,
        attempt,
        compact=batch_config.flat_single_case,
    )
    ...
    try:
        ...
        case_dir = (
            run_dir
            if batch_config.flat_single_case
            else run_dir / "cases" / _safe_id(case.case_id)
        )
        ...
        case_dir = (
            expected_run_dir
            if batch_config.flat_single_case
            else expected_run_dir / "cases" / _safe_id(case.case_id)
        )
        return ExternalAttemptResult(
            attempt=attempt,
            max_attempts=request.max_attempts,
...
def run_external_cases(
    ...
    batch_config = _build_batch_config(
        ...
        generation_dir,
        tuple(command),
        compact_single_case=len(cases) == 1,
    )
    ...
    _persist_result(generation_dir, initial, request.persist_report_text)
    ...
    with ThreadPoolExecutor(max_workers=request.parallel) as executor:
        while ready or in_flight:
            ...
            if launched:
                ...
                _persist_result(generation_dir, snapshot, request.persist_report_text)
    ...
    _persist_result(generation_dir, result, request.persist_report_text)
